...
/** \brief Set the Asic Register with value
 *        Warning:
 ...
 *        -2 - command fails unexpectedly
 */
DLL_API_C_DECL int32_t moSetAsicRegisterValue(IN const MO_CAMERA_HANDLE hCameraHandle,
                                              IN const uint16_t u16RegisterAddress, IN const uint16_t u16RegisterValue);

/** \brief Get the Asic Register with value
 *        Warning:
 *        Only used for specific developing purpose.
 *        Instructions from hardware department is required.
 *
 ...
 *        -2 - command fails unexpectedly
 */
DLL_API_C_DECL int32_t moGetAsicRegisterValue(IN const MO_CAMERA_HANDLE hCameraHandle,
...
// DLL_API_C_DECL int getAeStaticValues(IN const MO_CAMERA_HANDLE)
#ifdef __cplusplus
}
#endif
