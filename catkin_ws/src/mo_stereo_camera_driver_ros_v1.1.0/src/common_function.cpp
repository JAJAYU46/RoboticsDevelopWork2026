...
int32_t GetDisparityImageSelfAdaption(IN const uint64_t u64Height, IN const uint64_t u64Width, IN const uint64_t u16Step,
                                        IN const uint16_t* pu16DepthData, uint8_t* const pu8DepthImage)
{
    int32_t s32Result = 0;
    uint32_t u32DispCount[8192];
    memset(u32DispCount, 0, 8192*sizeof(uint32_t));

    do {
        if ((0 >= u64Width) || (0 >= u64Height) || (0 >= u16Step) || (nullptr == pu16DepthData) ||
            (nullptr == pu8DepthImage)) {
            s32Result = -1;
            break;
        }

        uint32_t u32TotalDisparityValueCount = 0;
        for (uint32_t u32Idx = 0; u32Idx < u64Height * u64Width; u32Idx++){
            uint16_t u16RawDisparityValue = pu16DepthData[u32Idx] & ((unsigned short)0x1fff);
            if (u16RawDisparityValue > 0)
            {
                u32DispCount[u16RawDisparityValue]++;
                u32TotalDisparityValueCount++;
            }
        }
        uint16_t u16PrevIndex = 0;
        for (uint16_t u16Idx = 1; u16Idx < 8192; u16Idx++)
        {
            if (u32DispCount[u16Idx] > 0)
            {
                u32DispCount[0] += u32DispCount[u16Idx];
                u32DispCount[u16Idx] = (float)u32DispCount[0] / u32TotalDisparityValueCount * 255;
                if (u16PrevIndex > 0)
                {
                    const uint16_t u16Difference = u32DispCount[u16Idx] - u32DispCount[u16PrevIndex];
                    if (u16Difference > (u16Idx - u16PrevIndex)/16 + 1)
                    {
                        u32DispCount[u16Idx] = u32DispCount[u16PrevIndex] + (u16Idx - u16PrevIndex)/16 + 1;
                    }
                }
                u16PrevIndex = u16Idx;
            }
        ...
        unsigned char* pu8ImageData = pu8DepthImage;
        for (uint64_t u64Idx = 0; u64Idx < u64Height * u64Width; u64Idx++){
            uint32_t u32RawDisparityValue = (pu16DepthData[u64Idx]) & ((unsigned short)0x1fff);
            if (u32RawDisparityValue == 0){
                *(pu8ImageData++) = 0;
                *(pu8ImageData++) = 0;
                *(pu8ImageData++) = 0;
            }
            else{
                if (u32DispCount[u32RawDisparityValue] == 0)
                {
                    u32DispCount[u32RawDisparityValue]++;
                }
                uint32_t u32Index = u32DispCount[u32RawDisparityValue]*15/2+128;
                *(pu8ImageData++) = g_caColorTable[u32Index * 3 + 2];
                *(pu8ImageData++) = g_caColorTable[u32Index * 3 + 1];
                *(pu8ImageData++) = g_caColorTable[u32Index * 3];
            }
        }
