...
bool g_bEnableDisplay = true;  // 是否显示左右 rectify 画面（无显示器/SSH 环境请设为 false）
...
void* publishThread(void*) {
    pthread_setname_np(pthread_self(), "img_pub");
    while (m_bIsRunnig) {
...
void* getIMGProcess(void* phCameraHandle) {
    pthread_setname_np(pthread_self(), "img_get");
    int32_t  n32Result        = 0;
    ...
    // --- profiling accumulators (debug only, remove after diagnosis) ---
    static uint64_t s_pollCount = 0, s_rectifyCount = 0;
    static double   s_pollUs = 0.0, s_rectifyUs = 0.0;
    static struct timespec s_lastReport = {0, 0};
    ...
    while (m_bIsRunnig) {
        ...
        s_pollUs += (t1.tv_sec - t0.tv_sec) * 1e6 + (t1.tv_nsec - t0.tv_nsec) / 1e3;
        s_pollCount++;
        ...
        clock_gettime(CLOCK_MONOTONIC, &t1);
        s_rectifyUs += (t1.tv_sec - t0.tv_sec) * 1e6 + (t1.tv_nsec - t0.tv_nsec) / 1e3;
        s_rectifyCount++;

        // 每 5 秒印一次統計，然後歸零
        clock_gettime(CLOCK_MONOTONIC, &t1);
        if (t1.tv_sec - s_lastReport.tv_sec >= 5) {
            printf("[PROFILE] poll: %lu calls, %.1f us/call total, %.2f ms/s busy | "
                   "rectify: %lu calls, %.1f us/call, %.2f ms/s busy\n",
                   s_pollCount, s_pollCount ? s_pollUs / s_pollCount : 0.0, s_pollUs / 1000.0,
                   s_rectifyCount, s_rectifyCount ? s_rectifyUs / s_rectifyCount : 0.0, s_rectifyUs / 1000.0);
            s_pollCount = s_rectifyCount = 0;
            s_pollUs = s_rectifyUs = 0.0;
            s_lastReport = t1;
        }
...
void* imuPublishThread(void*) {
    pthread_setname_np(pthread_self(), "imu_pub");
    while (m_bIsRunnig) {
...
void* getIMUProcess(void* phCameraHandle) {
    pthread_setname_np(pthread_self(), "imu_get");
    int32_t      n32Result  = 0;
